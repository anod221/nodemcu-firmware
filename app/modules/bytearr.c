// see https://github.com/anod221/lua-buffer/blob/master/bytearr.c
...
static inline int getNativeEndian()
{
  if( nativeEndian < 0 )
    {
      int x = 1;
...
static ICACHE_FLASH_ATTR int lbytearr_create( lua_State *L )
{
...
static ICACHE_FLASH_ATTR int lbytearr_init( lua_State *L )
{
  int n = lua_gettop( L );
  if( n < 1 ) {
    luaL_argerror(L, 1, MSG_INVALIDTYPE);
...
static ICACHE_FLASH_ATTR int lbytearr_load( lua_State *L )
{
...
static ICACHE_FLASH_ATTR int lbytearr_tostring( lua_State *L )
{
...
static ICACHE_FLASH_ATTR int lbytearr_readbytes( lua_State *L )
{
  Buf *p, *bytes;
...
static ICACHE_FLASH_ATTR int lbytearr_writebytes( lua_State *L )
{
  Buf *p, *bytes;
  size_t offset, length;
...
#define LUA_BIND_BUILDIN_WRITER( NAME, FUNC, CHECKF, TYPE )	\
  static ICACHE_FLASH_ATTR int lbytearr_##NAME( lua_State *L )	\
  {								\
    check_userdata_self(L);					\
								\
    Buf *p = lua_tobuffer(L, 1);				\
...
static ICACHE_FLASH_ATTR int lbytearr_readstr( lua_State *L )
{
  check_userdata_self(L);
...
static ICACHE_FLASH_ATTR int lbytearr_writestr( lua_State *L )
{
  check_userdata_self(L);
...
static ICACHE_FLASH_ATTR int lbytearr_clear( lua_State *L )
{
  check_userdata_self(L);
...
static ICACHE_FLASH_ATTR int lbytearr_slice( lua_State *L )
{
  check_userdata_self(L);
...
static ICACHE_FLASH_ATTR int lbytearr_getlen( lua_State *L )
{
  check_userdata_self(L);
...
static ICACHE_FLASH_ATTR int lbytearr_getter( lua_State *L )
{
  check_userdata_self(L);
...
static ICACHE_FLASH_ATTR int lbytearr_setter( lua_State *L )
{
  check_userdata_self(L);
...
static ICACHE_FLASH_ATTR int lbytearr_gc( lua_State *L )
{
  check_userdata_self(L);
