...
// ======== for initialize ========
static inline void rc522_pin_init( u8 v )
{
...
void rc522_init( rfid_dev dev )
{
  ...
  u8 n = rc522_readreg(dev, VersionReg).bRegister;
  c_printf("ver: %xh", n);
  return;
  // reset all register
  rc522_reset( dev );
...
#define SPI_BITSIZE (sizeof(u8) * 8)

static inline rc522_spiaddr rc522_address( u8 addr, u8 type )
{
  spiaddr spi = { addr, type };
  rc522_spiaddr retval;
  retval.spiaddr = spi;
  return retval;
}
...
void rc522_writereg( rfid_dev dev, u8 addr, Rc522Reg val )
{
  ...
  spi_data_type data = (rc522_address(addr, SPI_WRITE).addr << SPI_BITSIZE) | val.bRegister;
...
Rc522Reg rc522_readreg( rfid_dev dev, u8 addr )
{
  ...
  spi_data_type data = rc522_address(addr, SPI_READ).addr;
  data <<= SPI_BITSIZE;
...
// ======== for send command ========
static inline void clear_fifo(rfid_dev dev)				
{						
...
static int to_fifo(rfid_dev dev, u8 *buff, size_t sz)
{
  ...
  // send addr
  spi_data_type t = rc522_address(FIFODataReg, SPI_WRITE).addr;
...
static size_t from_fifo(rfid_dev dev, u8 *buff)
{
  ...
  // init array
  int id = dev->spi_id;
  rc522_spiaddr regaddr = rc522_address(FIFODataReg, SPI_READ);
...
static inline void wait_transmit_done( rfid_dev dev)
{					      
...
static inline void wait_receive_done( rfid_dev dev)
{					      
  bStatus2Reg bst = readreg(dev, Status2Reg);
  while(0 != (bst.ModemState & 0x04)){	      
    system_soft_wdt_feed();		      
...
static int rc522_cmd_transceive(rfid_dev dev, u8 *arg, size_t szarg, u8 *res)
{
  ...
  c_printf("get the status:%xh", rc522_readreg(dev, FIFOLevelReg));
  return -10000;
